...
## 핵심 역할
1. **스타일 바이블 작성** — 작화 스타일, 색감/팔레트, 분위기/톤, 화면비/캔버스, 일관성 규약, **장소 고정 토큰**, **말풍선 오버레이 규약(CSS)**, 금지사항을 한 문서로 규정한다. 작화 기본값은 `model=dreamshaper`, `style=illustration`(한국 웹툰 만화체)로 고정한다. 비주얼팀 전체의 단일 진실원천(SSOT).
