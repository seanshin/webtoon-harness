...
3. 패널마다 `position:relative`로 감싼 컨테이너 안에 `<img>`(원본 비율, 폭 100%·높이 auto)를 깔고, 그 위에 lettering.md의 각 말풍선을 `position:absolute` div로 얹는다:
   ```html
   <div class="panel">
     <img src="data:image/png;base64,iVBORw0KGgo...">   <!-- 패널 PNG를 base64 임베드 -->
     <div class="bubble tail-bl" style="top:6%; left:6%; max-width:58%;">여기가… 맞나?</div>
   </div>
   ```
   - **`img`는 패널 PNG를 base64 data URI로 임베드한다.** `../../05_panels/...` 상대경로는 일부 브라우저/webview가 `../` 상위 탐색을 막아 이미지가 깨진다(실제 발생). 임베드하면 자가완결 단일 HTML이 되어 어디서 열어도·이동·공유해도 안 깨진다.
...
```
RELEASE/ep{NN}/
  index.html          # 자가완결 뷰어(패널 PNG를 base64 임베드 — 외부 참조 0)
